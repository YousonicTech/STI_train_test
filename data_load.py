...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        # mean = 100, sqrt = 50, so, pts should be (pts - 00)/50
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        image, ddr, t60, meanT60, valid_len = sample['image'], sample['ddr'], sample['t60'], sample["MeanT60"], sample[
            'validlen']
        image = np.expand_dims(image, 0)
        ddr = ddr.astype(float)
        t60 = t60.astype(float)
        meanT60 = meanT60.astype(float)
        valid_len = valid_len.astype(int)
        ...
        return {'image': torch.from_numpy(image),
                ...
                'validlen': torch.from_numpy(valid_len)}
